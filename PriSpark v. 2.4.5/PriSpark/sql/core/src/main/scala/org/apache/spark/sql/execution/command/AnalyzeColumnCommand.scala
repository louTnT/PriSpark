...
//    println("attributesToAnalyze: " + attributesToAnalyze)
//    println("attributesToAnalyze.length: " + attributesToAnalyze.length)
...
//    val expressions = attributesToAnalyze.head +:
//      attributesToAnalyze.map(statExprs(_, conf, attributePercentiles))
...
//    println("namedExpressions: " + namedExpressions)

    val statsRow = new QueryExecution(sparkSession, Aggregate(Nil, namedExpressions, relation))
      .executedPlan.executeTake(1).head
//    println("statsRow: " + statsRow)

//    println("attributesToAnalyze: " + attributesToAnalyze)

    val expressions_Fre = attributesToAnalyze.map(statExprs_Fre(_, conf))
//    println("expressions_Fre: " + expressions_Fre)
    val namedExpressions_Fre = expressions_Fre.map(e => Alias(e, e.toString)())
//    println("namedExpressions_Fre: " + namedExpressions_Fre)

    val statsRow_Fre = new QueryExecution(sparkSession, GlobalLimit(Literal(1), LocalLimit(Literal(1), Sort(Seq(SortOrder(Count(Literal(1)).toAggregateExpression(), Descending, NullsLast, Set(Count(Literal(1))))), true, Aggregate(attributesToAnalyze, namedExpressions_Fre, relation)))))
        .executedPlan.executeTake(1).head
//    println("~~~~~~~~~~~~~" + statsRow_Fre)

    val maxFre = statsRow_Fre.getLong(0)
//    println("maxFre: " + maxFre)
//    println("test: " + statsRow.getStruct(1, 8))
//    val Fre = statsRow.getStruct(1, 8).getLong(6)
//    println("Fre: " + Fre)
    ...
    val columnStats = attributesToAnalyze.zipWithIndex.map { case (attr, i) =>
      ...
      (attr.name, rowToColumnStat(statsRow.getStruct(i + 1, 7), attr, rowCount, maxFre,
        attributePercentiles.get(attr), attributesToAnalyze, sparkSession, relation).toCatalogColumnStat(attr.name, attr.dataType))
    }.toMap

//    println("statsRow.getStruct(i + 1, 7): " + statsRow.getStruct(0 + 1, 7))
//    println("statsRow.getStruct(i + 1, 7).(0): " + statsRow.getStruct(0 + 1, 7).getLong(0))
//    println("columnStats: " + columnStats)

    (rowCount, columnStats)
  }

  private def statExprs_Fre(col: Attribute, conf: SQLConf): Expression = {
    val MaxFre = Count(Literal(1)).toAggregateExpression()
    MaxFre
  }
...
//    println("Seq(HyperLogLogPlusPlus(col, conf.ndvMaxError), numNonNulls): " + Seq(HyperLogLogPlusPlus(col, conf.ndvMaxError), numNonNulls))
//    println("ndv: " + ndv)
//    val MaxFre = Max(Count(Grouping(Cube(Seq(col)))))
//    val MaxFre = Count(col)
...
//      println("intervalNdvsExpr: " + intervalNdvsExpr)
//      println("Literal(colPercentiles(col), ArrayType(col.dataType)): " + Literal(colPercentiles(col), ArrayType(col.dataType)))
  ...
  private def rowToColumnStat(
    ...
    rowCount: Long,
    Fre: Long,
    percentiles: Option[ArrayData],
    attributesToAnalyze: Seq[Attribute],
    sparkSession: SparkSession,
    relation: LogicalPlan): ColumnStat = {
...
//      println("ndvs0: " + ndvs(0))
//      println("ndvs1: " + ndvs(1))
//      println("ndvs2: " + ndvs(2))
      ...
      val expressions_Fre_bin = attributesToAnalyze.map(statExprs_Fre(_, conf))
      val namedExpressions_Fre_bin = expressions_Fre_bin.map(e => Alias(e, e.toString)())

      var maxFre_bin = new Array[Long](conf.histogramNumBins)
      for (i <- 0 to (conf.histogramNumBins - 1)){
//        val lo = endpoints(i)
//        val hi = endpoints(i + 1)

        val statsRow_Fre_bin = new QueryExecution(sparkSession, GlobalLimit(Literal(1), LocalLimit(Literal(1), Sort(Seq(SortOrder(Count(Literal(1)).toAggregateExpression(), Descending, NullsLast, Set(Count(Literal(1))))),
          true, Aggregate(attributesToAnalyze, namedExpressions_Fre_bin, Filter(And(GreaterThanOrEqual(attributesToAnalyze(0), Literal(endpoints(i))),LessThanOrEqual(attributesToAnalyze(0), Literal(endpoints(i + 1)))), relation))))))
          .executedPlan.executeTake(1).head

        maxFre_bin(i) = statsRow_Fre_bin.getLong(0)
      }

//      maxFre_bin.foreach(i => println(i))
...
//      }
      ...
      val bins = maxFre_bin.zipWithIndex.map { case (mfb, i) =>
        HistogramBin(endpoints(i), endpoints(i + 1), mfb)
      }
