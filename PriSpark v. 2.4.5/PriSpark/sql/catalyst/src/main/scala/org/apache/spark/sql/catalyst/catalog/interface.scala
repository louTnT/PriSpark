...
case class CatalogColumnStat(
  ...
  def toMap(colName: String): Map[String, String] = {
    ...
    maxFre.foreach { v => map.put(s"${colName}.${CatalogColumnStat.KEY_MAX_FRE}", v.toString) }
...
object CatalogColumnStat extends Logging {
  ...
  private val KEY_MAX_FRE = "maxFre"
  ...
  def fromMap(
    ...
    try {
      Some(CatalogColumnStat(
        ...
        maxFre = map.get(s"${colName}.${KEY_MAX_FRE}").map(_.toLong),
